/// @file render_hybrid.cpp
/// @brief Test for the Vulkan hybrid raster + ray-traced renderer.
///
/// Renders a Cornell Box scene using the hybrid pipeline:
///   G-buffer pass (raster) -> ray-traced shadows & reflections ->
///   compute lighting -> SSAO -> bloom -> tone mapping -> FXAA.
///
/// Verifies the output is non-trivial and checks that ray-traced
/// effects produce reasonable results (correct brightness range,
/// spatial variance, non-black coverage, tone-mapped output).
///
/// Requires ZS_ENABLE_VULKAN=1 and a Vulkan-capable GPU with
/// VK_KHR_ray_tracing_pipeline support at runtime.
...
// -------------------------------------------------------------------
// Helper: count fully clipped (pure white) pixels
// -------------------------------------------------------------------
...
// -------------------------------------------------------------------
// Test: hybrid renderer on Cornell box
// -------------------------------------------------------------------

static void test_hybrid_cornell_box() {
  auto renderer = createVulkanHybridRenderer();
  if (!renderer) {
    std::printf("[SKIP] Vulkan hybrid renderer not available "
                "(no Vulkan or no RT support) — skipping hybrid test\n");
    return;
  }

  bool ok = renderer->init();
  if (!ok) {
    std::printf("[SKIP] Vulkan hybrid renderer init failed — skipping hybrid test\n");
    return;
  }

  std::printf("[hybrid] renderer: %s\n", renderer->name());
  ...
  std::printf("[hybrid] scene: %zu instances, %zu materials, %zu lights\n",
  ...
  req.method = RenderMethod::Hybrid;

  // Render with full hybrid pipeline
  RasterResult result = renderer->render(req, 0);
  assert(result.success && "Hybrid render must succeed");
  assert(!result.color.data.empty() && "Hybrid result must have color data");
  assert(result.color.width == 512 && result.color.height == 512);

  std::printf("[hybrid] render time: %.1f ms\n", result.render_time_us / 1000.0);

  // Save PNG
  std::filesystem::create_directories(k_output_dir);
  std::string path = std::string(k_output_dir) + "/hybrid_cornell_box.png";
  bool wrote = writePNG(path,
                        result.color.data.data(),
                        result.color.width, result.color.height);
  assert(wrote && "writePNG must succeed");
  std::printf("[hybrid] wrote: %s\n", path.c_str());
  ...
  std::printf("[hybrid] average brightness: %.1f / 255\n", avg);
  assert(avg > 5.0 && "Hybrid image must not be all black");
  assert(avg < 250.0 && "Hybrid image must not be all white");

  // 2. Brightness variance — should have contrast (shadows + reflections)
  double variance = brightnessVariance(pixels, w, h, avg);
  std::printf("[hybrid] brightness variance: %.1f\n", variance);
  assert(variance > 50.0 && "Hybrid image must have contrast");
  ...
  std::printf("[hybrid] non-black pixel ratio: %.2f%%\n", nonBlackRatio * 100.0);
  ...
  std::printf("[hybrid] clipped pixel ratio: %.2f%%\n", clippedRatio * 100.0);
  assert(clippedRatio < 0.05 && "Tone mapping should prevent most pure-white clipping");

  renderer->shutdown();
  std::printf("[PASS] test_hybrid_cornell_box\n");
}
...
int main() {
  std::printf("=== Render Hybrid RT Tests ===\n");

  test_hybrid_cornell_box();

  std::printf("=== Hybrid RT tests complete ===\n");
  return 0;
}
