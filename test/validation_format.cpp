...
int main() {
  ...
  record.target = "gpu0";
  record.note = "line1\nline2";
  record.set_metadata("audio.sampleRate", "48000");
  record.set_metadata("audio.bufferSize", "128");
  ...
  assert(recordJson.find("\"metadata\":[{\"key\":\"audio.sampleRate\",\"value\":\"48000\"},{\"key\":\"audio.bufferSize\",\"value\":\"128\"}]") != std::string::npos);
  ...
  assert(reportJson.find("\"metadata\":[{\"key\":\"profile\",\"value\":\"runtime\"}]") != std::string::npos);
  ...
  assert(summaryText.find("metadata.audio.sampleRate=48000") != std::string::npos);
  ...
  assert(reportText.find("metadata.profile=runtime") != std::string::npos);
  assert(reportText.find("metadata.audio.bufferSize=128") != std::string::npos);
