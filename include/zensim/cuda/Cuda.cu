...
namespace zs {
  ...
  void *Cuda::CudaContext::streamMemAlloc(size_t size, void *stream, const source_location &loc) {
    void *ptr;
    if (stream == nullptr) {
      // to avoid cuMemAllocAync failure on Linux paltforms.
      checkError(cuMemAlloc((CUdeviceptr *)&ptr, size));
    } else {
      checkError(cuMemAllocAsync((CUdeviceptr *)&ptr, size, (CUstream)stream));
    }
    return ptr;
  }
  void Cuda::CudaContext::streamMemFree(void *ptr, void *stream, const source_location &loc) {
    if (stream == nullptr) {
      // to avoid cuMemAllocAync failure on Linux paltforms.
      checkError(cuMemFree((CUdeviceptr)ptr));
    } else {
      checkError(cuMemFreeAsync((CUdeviceptr)ptr, (CUstream)stream));
    }
  }
  ...
  void Cuda::CudaContext::tock(Cuda::CudaContext::StreamExecutionTimer *timer,
                               const source_location &loc) {
    checkError(cuLaunchHostFunc((CUstream)timer->stream, (CUhostFn)recycle_timer, (void *)timer));
  }
