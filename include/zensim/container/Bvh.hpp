...
namespace zs {
  ...
  struct LBvhView<space_, const LBvhT, Base> {
    ...
    // template <size_t CDIM, typename VecT, class F>
    // constexpr auto find_nearest_projection(const VecInterface<VecT> &p, F &&f) const {
    //   using T = typename VecT::value_type;
    //   index_t idx = -1;
    //   T dist = detail::deduce_numeric_max<T>();
    //   zs::vec<T, CDIM> bary{};
    ...
    //       }
    //     }
    //     return zs::make_tuple(idx, bary, dist);
    //   }
    ...
    //     // leaf node check
    //     if (level == 0) {
    //       if (auto d = distance(p, getNodeBV(node)); d < dist) f(_auxIndices[node], dist, bary, idx);
    //       node++;
    //     } else  // separate at internal nodes
    //       node = _auxIndices[node];
    //   }
    ...
    template<typename VecT, typename VecW, typename Ti, typename T = typename VecT::value_type,  class F>
    constexpr void find_nearest_projection(const VecInterface<VecT> &pos, Ti &outIdx, T& dist, VecW& bary, F &&f) const {
        Ti node = 0;
        VecW wsTmp{};
        while (node != -1 && node != numNodes()) {
          Ti level = _levels[node];
          // level and node are always in sync
          for (; level; --level, ++node)
            if (auto d = distance(getNodeBV(node), pos); d > dist)
              break;
          // leaf node check
          if (level == 0) {
            const auto eid =_auxIndices[node];
            T d{};
            f(pos, eid, d, wsTmp);
            if (d < dist) {
              outIdx = eid;
  ...
  /// @note F return_value indicates early exit
  template <typename BV, class F> constexpr void iter_neighbors_dynamic(BV &bv, F &&f) const {
        if (auto nl = numNodes(); nl <= 2) {
          for (index_t i = 0; i != nl; ++i) {
            if (overlaps(getNodeBV(i), bv)) {
              if constexpr (is_same_v<decltype(declval<F>()(declval<index_t>(), declval<BV&>())), void>)
                f(i, bv);
              else {
                if (f(i, bv)) return;
              }
        ...
        while (node != -1 && node != _numNodes) {
          ...
          if (level == 0) {
            if (overlaps(getNodeBV(node), bv)) {
              if constexpr (is_same_v<decltype(declval<F>()(declval<index_t>(), declval<BV&>())), void>)
                f(_auxIndices[node], bv);
              else {
                if (f(_auxIndices[node], bv)) return;
              }
            }
            node++;
          } else  // separate at internal nodes
