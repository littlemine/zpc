...
namespace zs {
  ...
  template <typename T,
          enable_if_all<is_same_v<T, remove_cvref_t<T>>, zs::is_default_constructible_v<T>,
                        zs::is_trivially_copyable_v<T>>
          = 0>
  auto to_std_vector(const Vector<T> &v, std::vector<T> &ret) {
    Resource::copy(MemoryEntity{MemoryLocation{memsrc_e::host, -1}, (void *)ret.data()},
                   MemoryEntity{v.memoryLocation(), (void *)v.data()}, 
                   sizeof(T) * v.size());
    return ret;
  }
  ...
  template <typename S, typename T> void serialize(S &s, Vector<T, ZSPmrAllocator<>> &c) {
    using C = Vector<T, ZSPmrAllocator<>>;
    ...
    if constexpr (sizeof(typename C::value_type) <= 16)
      s.template container<sizeof(typename C::value_type)>(
          c, detail::deduce_numeric_max<typename C::size_type>());
    else
