...
#  endif
  ...
  constexpr u64 expand_bits_6d(u32 v) noexcept {
    // expands lower 10 bits to 60 bits (stride 6, i.e., insert 5 zeros between each bit)
    // Pattern: bit at position i goes to position i*6
    // Masks: take every 6th bit starting at positions 0,6,12,18,...
    u64 x = v & 0x3FF;  // keep lower 10 bits
    x = (x | (x << 25)) & 0x1F0000001FULL;       // _____ xxxxx _____________________________ xxxxx
    x = (x | (x << 10)) & 0x1C0070001C0007ULL;   // ___xxx ___xxx ___xxx ___xxx ___xxx ___xxx x
    x = (x | (x << 5))  & 0x1041041041041041ULL; // _x____x____x____x____x____x____x____x____x____x
    return x;
  }
  ...
  constexpr u64 morton_6d_64(float x, float y, float z, float u, float v, float w) noexcept {
    // Interleave 6 coordinates: each expanded to stride-6, then shift by 0,1,2,3,4,5
    return expand_bits_6d((u32)(x * 1024.f))
           | (expand_bits_6d((u32)(y * 1024.f)) << 1)
           | (expand_bits_6d((u32)(z * 1024.f)) << 2)
           | (expand_bits_6d((u32)(u * 1024.f)) << 3)
           | (expand_bits_6d((u32)(v * 1024.f)) << 4)
           | (expand_bits_6d((u32)(w * 1024.f)) << 5);
  }
  template <typename T>
  constexpr auto morton_6d(T x, T y, T z, T u, T v, T w) noexcept {
    return morton_6d_64((float)x, (float)y, (float)z, (float)u, (float)v, (float)w);
  }
  ...
  template <int dim, typename Vec, enable_if_type<(dim == 6), char> = 0>
  constexpr auto morton_code(const Vec &coord) noexcept {
    return morton_6d(coord[0], coord[1], coord[2], coord[3], coord[4], coord[5]);
  }
#if 0
  ...
  /**
   */
  template <typename Tn = zs::size_t> constexpr Tn interleaved_bit_mask(int dim) noexcept {
