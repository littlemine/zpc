# 3C System — 6-Week Milestone Plan
...
2. **Module decomposition** — define top-level modules (`3c_input`, `3c_camera`,
   `3c_character`, `3c_core`) and their dependency graph. Define frame update order.
...
4. **Validation layout** — define validation scenario descriptors and artifact directory
   structure (`test/3c/`, `zpc_assets/3c/baselines/`).
5. **Architecture document** — consolidate all decisions into `docs/3c_architecture.md`.
...
2. Total 3C frame budget ≤ 2 ms (camera + character + input combined)
3. API documentation — all public types and functions documented
4. Integration guide — how to consume 3C from an application
