# ZPC 3C Integration Guide

How to consume the 3C (Camera, Character, Control) system from an application.
...
The fastest way to get 3C running is through `ThreeCPipeline`, which wires
...
| Target | Use case |
|--------|----------|
| `zpc_3c_core` | Math extensions, `FrameContext`, quaternion utilities only |
| `zpc_3c_input` | Input system (action maps, axis processing, input buffer) |
| `zpc_3c_camera` | Camera modes, matrix builders, frustum, shake, rig |
| `zpc_3c_character` | Character state machine, locomotion, jump mechanics |
| `zpc_3c_integration` | Full pipeline — links all of the above |
...
target_link_libraries(my_app PRIVATE zpc_3c_integration)

# Camera only (no character)
add_executable(cam_viewer viewer.cpp)
target_link_libraries(cam_viewer PRIVATE zpc_3c_camera)
```
...
zpcbase
  └── zpc_3c_core
        ├── zpc_3c_input
        ├── zpc_3c_camera
        └── zpc_3c_character (also links zpc_3c_input)
              └── zpc_3c_integration (links camera + character + input)
```
...
```cpp
#include "zensim/3c/camera/CameraMode.hpp"
...
```cpp
#include "zensim/3c/input/AxisProcessor.hpp"
...
The 3C system does **not** own physics. You provide ground info each frame:
...
`ThreeCPipeline` is the recommended entry point for applications that need
