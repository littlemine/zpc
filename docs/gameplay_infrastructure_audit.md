# Gameplay Infrastructure Audit — ZPC Codebase
...
what infrastructure is available for building a Gameplay (Camera, Character, Control) system
and what gaps must be filled.

**Verdict:** ZPC provides strong math primitives, a full validation/benchmark pipeline,
canary scenario infrastructure, and a clean CMake module pattern. No Gameplay-specific code
...
Gameplay feel-metrics (frame time, input-to-screen latency, smoothness, overshoot) can
...
Gameplay scenarios (e.g., "orbit camera 360° at 60 FPS", "sprint + jump sequence") can
...
The execution graph could model the Gameplay frame update pipeline (input → character → camera)
...
New Gameplay modules should follow this pattern exactly:
- `zpc_gameplay_core` — math extensions, data types
- `zpc_gameplay_input` — input abstraction
- `zpc_gameplay_camera` — camera types and controllers
- `zpc_gameplay_character` — character state and movement
- `zpc_gameplay_validation` — Gameplay-specific validation scenarios
...
Gameplay tests should follow these conventions exactly.
...
ZPC is well-suited as a foundation for Gameplay. The validation pipeline is production-ready
for feel-metrics. The canary system handles scenario management. Math primitives cover
vector/quaternion basics but need `slerp`. The main engineering work is defining the Gameplay
