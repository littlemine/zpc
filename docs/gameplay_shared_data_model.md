# Gameplay Shared Data Model

**Date:** 2026-03-13
**Branch:** `zpc_3c`

This document defines the minimal shared data types that flow between the Gameplay modules.
These are the "wire types" — the structs that cross module boundaries at each frame.

---

## 1. Core Types (`gameplay/Core.hpp`)
...
namespace zs::gameplay {
  ...
  // Shorthand aliases for Gameplay domain
  ...
  /// Per-frame context passed to all Gameplay update phases
  struct FrameContext {
...
## 2. Input Types (`gameplay/input/InputTypes.hpp`)
...
## 3. Character Types (`gameplay/character/CharacterState.hpp`)
...
## 4. Camera Types (`gameplay/camera/Camera.hpp`)
...
## 5. Telemetry Types (`gameplay/Telemetry.hpp`)
...
namespace zs::gameplay {
  ...
  ///
  ///   "gameplay.total_phase_ms"         → totalPhaseMs     (threshold: ≤ 2.0)
  ///   "gameplay.input_latency_ms"      → inputToScreenLatencyMs (threshold: ≤ 16.67)
  ///   "gameplay.camera_jitter_px_rms"  → cameraJitterPx   (threshold: ≤ 0.5)
  ///   "gameplay.camera_phase_ms"       → cameraPhaseMs    (threshold: ≤ 0.5)
  ///   "gameplay.character_phase_ms"    → characterPhaseMs  (threshold: ≤ 0.5)
...
5. **Animation tags as bitmask** — `animTags` is a simple `u32` bitmask. The Gameplay system
   sets tags; the animation system consumes them. No animation logic lives in Gameplay.
